...
function execute_batch_double_args(args; file_name, max_force=20.0, max_steps=200)
    cmd_args = parse_args(args)
    b = double_pendulum_batch(command_args=cmd_args)
    ...
    data = Environments.PendulumData(gravity, masscart, masspole, total_mass, length, polemass_length, max_force, tau, kinematics_integrator)
...
function double_pendulum_batch(; command_args::CommandArgs)
